...
if __name__ == "__main__":
    ...
    for iops_file in iops_files:
        ...
        for key, values in data_iops.items():
            ramdisk_values = values['ramdisk']
            tmpfs_values = values['tmpfs']

            for i in range(len(ramdisk_values)):
                average = (ramdisk_values[i] + tmpfs_values[i]) / 2
                data_iops[key]['fuse_boc'][i] = average
    ...
    for latency_file in latency_files:
        ...
        for key, values in data_lat.items():
            ...
            for i in range(len(ramdisk_values)):
                average = (ramdisk_values[i] + tmpfs_values[i]) / 2
                data_lat[key]['fuse_boc'][i] = average
